...
import { chartG2Req,g6Req,l7Req } from '@/service';
...
interface chartModelType {
  ...
  reducers: {
    /*  CHART: ImmerReducer<chartModelStateType>; */
  };
}

const cahrtModel: chartModelType = {
  namespace: 'chart',
  state: {},
  effects: {
    *getG2Data({ payload }, { call, put }) {
      let data = yield call(chartG2Req);
      console.log(data);
    ...
    *getG6Date({payload},{call,put}){
      let data= yield call(g6Req)
   ...
   /*  *getL7Date({payload},{call,put}){
      console.log(yield call(l7Req));
